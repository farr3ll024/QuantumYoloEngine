...
"""Plotly figure builders. Pure data → figure."""
...
BASE = dict(
    ...
    legend=dict(
        bgcolor="rgba(15,19,24,0.9)", bordercolor=BORDER, borderwidth=1,
    ...
    hovermode="x unified",
    hoverlabel=dict(
        bgcolor="#151c24", bordercolor=BORDER,
...
def _nearest(px_df, ts):
    if px_df.empty: return None
...
def price_line_fig(prices, asset_focus, last_n):
    if prices.empty:
    ...
    for i, (pid, grp) in enumerate(df.groupby("product_id")):
        ...
        fig.add_trace(go.Scatter(
            x=grp["ts"], y=grp["price"], name=str(pid), mode="lines",
            line=dict(color=COLORWAY[i % len(COLORWAY)], width=1.5),
            hovertemplate=f"{pid}  $%{{y:,.2f}}<extra></extra>",
        ))
    fig.update_layout(yaxis_tickprefix="$", yaxis_tickformat=",.0f")
    return fig


def price_candle_fig(prices, asset_focus, last_n, interval):
    ...
    fig.add_trace(go.Candlestick(
        ...
        low=ohlc["low"], close=ohlc["close"], name=asset_focus,
...
def add_trade_overlays(fig, prices, events, asset_focus):
    ...
    for et in sorted(m["et"].unique()):
        ...
        sub = m[m["et"] == et]
        fig.add_trace(go.Scatter(
            x=sub["ts"], y=sub["y"], mode="markers", name=et,
...
def trades_bar_fig(trades):
    ...
    fig = _fig(220)
    fig.add_trace(go.Bar(
        x=vc["exit_type"], y=vc["count"],
        marker=dict(color=colors, line=dict(width=0)),
        hovertemplate="%{x}: %{y}<extra></extra>",
    ))
    fig.update_layout(showlegend=False, bargap=0.4,
                      margin=dict(l=40, r=8, t=4, b=40))
    return fig


def equity_compare_fig(base_eq, cand_eq):
    ...
    for df, name, color in [(base_eq, "Baseline", AMBER), (cand_eq, "Candidate", CYAN)]:
        if "ts" in df.columns and "equity" in df.columns:
            fig.add_trace(go.Scatter(
                x=df["ts"], y=df["equity"].astype(float),
